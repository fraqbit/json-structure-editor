...
import { AttachmentDialogState, EntityCreationData, FilterOption, Filters, Group,  Marketplace, Resolution, StructuredData, Widget } from './types';
import CreationDialog from './CreationDialog';
import AttachmentDialog from './AttachmentDialog';

const applyFilters = (data: StructuredData, filters: Filters, searchTerm: string): StructuredData => {
  const lowerSearchTerm = searchTerm.toLowerCase();
  
  // Фильтрация виджетов
  const filteredWidgets = data.widgets.filter(widget => {
    const matchesSearch = !searchTerm || widget.code.toLowerCase().includes(lowerSearchTerm);
  ...
  const filteredGroups = data.groups.filter(group => {
    const matchesSearch = !searchTerm || group.code.toLowerCase().includes(lowerSearchTerm);
    const matchesFilters = Object.entries(filters).every(([key, values]) => {
      if (values.length === 0) return true;
      const [type, field] = key.split('_');
  ...
  const filteredMarketplaces = data.marketplaces.filter(mp => {
    const matchesSearch = !searchTerm || mp.code.toLowerCase().includes(lowerSearchTerm);
    const matchesFilters = Object.entries(filters).every(([key, values]) => {
      if (values.length === 0) return true;
      const [type, field] = key.split('_');
...
const App = () => {
  ...
  const openAttachmentDialog = (type: 'marketplace' | 'group', target: any) => {
    ...
    setAttachmentDialog({
      ...
      type,
      target,
      availableItems: type === 'marketplace' 
  ...
  const transformToOriginal = (structuredData: StructuredData | null): StructuredData | null => {
  ...
  const handleResolveConflicts = useCallback((resolution: Resolution) => {
    if (resolution.action === 'apply-all') {
      ...
      const targetConflict = conflicts.find(c => c.code === resolution.newCode);
  
      if (targetConflict) {
        switch (targetConflict.type) {
          case 'group':
            clonedData.groups.push({ ...targetConflict.new, code: resolution.newCode });
            break;
          case 'widget':
            clonedData.widgets.push({ ...targetConflict.new, code: resolution.newCode });
            break;
        }
      }
  ...
  const showSnackbar = (message: string, severity: 'info' | 'error' | 'success' | 'warning') => {
...
{structuredData && (
  <AttachmentDialog
    ...
    onClose={() => setAttachmentDialog({ ...attachmentDialog, open: false })}
    onSubmit={handleAttachItems}
    data={structuredData}
    type={attachmentDialog.type === 'marketplace' ? 'groups' : 'widgets'}
    title={attachmentDialog.type === 'marketplace' 
...
{structuredData && (
  <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
    <Button 
      variant="outlined" 
      onClick={() => setCreationDialog({ open: true, type: 'marketplace' })}
    >
      + Маркетплейс
    </Button>
    <Button 
