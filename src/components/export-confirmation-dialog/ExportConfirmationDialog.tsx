        ...
        <Dialog open={open} onClose={onCancel} maxWidth="md" fullWidth>
            <DialogTitle>
                ...
                <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                    ...
                    />
                    <Chip
                        label={`${changes.length} изменений`}
                        color="info"
                        size="small"
                    />
                </Box>
                ...
                <Box sx={{ pt: 2 }}>
                    {tabValue === 0 && (
                        errors.length > 0 ? (
                            ...
                            <Typography color="success.main" sx={{ p: 2 }}>
                                Ошибок валидации не обнаружено
                            </Typography>
                        )
                    )}
                            ...
                            <List dense>
                                {changes.map((change, index) => (
                                    <ListItem key={index}>
                                        ...
                                        <ListItemText
                                            primary={`${change.type}: ${change.code}`}
                    ...
                    {tabValue === 2 && (
                        <Box>
                            ...
                            <Typography>
                                • Добавлено: {changes.filter(c => c.action === 'added').length}
                            </Typography>
                            <Typography>
                                • Изменено: {changes.filter(c => c.action === 'modified').length}
                            </Typography>
                            <Typography>
                                • Удалено: {changes.filter(c => c.action === 'deleted').length}
                            </Typography>

                            {errors.length > 0 && (
                                <Typography color="error" sx={{ mt: 2 }}>
                                    Внимание: файл содержит ошибки валидации!
                                </Typography>
                            )}
                        </Box>
                    )}
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    startIcon={errors.length ? <WarningIcon /> : undefined}
                >
